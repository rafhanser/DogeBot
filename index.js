...
// RAFHAN SER
// ©RAFHAN SER RAFHAN SER Incorporated
const {
...
const { jadiRAFHAN SER, stopjadiRAFHAN SER, listjadiRAFHAN SER } = require("./lib/jadiRAFHAN SER");
...
const body = (type === 'listResponseMessage' && mek.message.listResponseMessage.title) ? mek.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && mek.message.buttonsResponseMessage.selectedButtonId) ? mek.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) : ""
    ...
    const RAFHAN SERNumber = RAFHAN SER.user.jid;
    
    const RAFHAN SERNumberss = RAFHAN SER.user.jid + "@c.us";
...
const checkRegisteredUser = (sender) => {
   ...
   const isRegister = register.includes(sender)
    ...
    const isRAFHAN SERGroupAdmins = groupAdmins.includes(RAFHAN SERNumber) || false;
    ...
    const totalhit = JSON.parse(fs.readFileSync("./lib/totalcmd.json"))[0]
      .totalcmd;
const daftar1 = `Hi ${pushname} ${ucapanWaktu} \n\nBefore Using the RAFHAN SER Verify First `
...
RAFHAN SER.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      RAFHAN SER.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = RAFHAN SER.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `RAFHAN SER RAFHAN SER Inc.`, "footerText": "*_RAFHAN SER_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
await RAFHAN SER.setStatus(`RAFHAN SER Mode On Since ${uptime}`).catch((_) => _);
...
𝗡𝗼𝘁𝗲 : Horny = Bonks💥
Just Learn to Make RAFHAN SERs 🐶
...
case 'suit':
              ...
              reply(`You choose Rock and Scissors RAFHAN SER\nYou win!`)
              } else {
              reply(`You choose Rock and RAFHAN SERs choose Paper\nYou lose!`)
}
              } else if ( userspilih == 'scissors' ) {
              if( computer == 'rock' ) {
              reply(`You choose Scissors and RAFHAN SERs choose Rock\nYou lose!`)
              } else {
              reply(`You choose Scissors and Paper RAFHAN SER\nYou win!`)
}
              } else if ( userspilih == 'paper' ) {
              if( computer == 'rock' ) {
              reply(`You choose Paper and Rock RAFHAN SER\nYou win!`)
              } else {
              reply(`You choose Paper and RAFHAN SERs choose Scissors\nYou lose`)
}
}
              break
case 'bugreport':
              ...
              teks = args.join(' ')
              reply('Thank you for reporting the bug to the owner, if its just a fad then it will be banned by a RAFHAN SER!')
...
case 'restart':
             ...
             reply(mess.wait)
             exec(`node main`)
             reply('_Restarting RAFHAN SER Success_')
break
case 'toxic':
              Toxic().then(toxic => {
              reply (toxic)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               buttons = [{buttonId: `mygithub`,buttonText:{displayText: 'SC RAFHAN SER'},type:1},{buttonId:`iggw`,buttonText:{displayText:'PM ON WHATSAPP'},type:1}]
...
case 'buglink':
               ...
               RAFHAN SER.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '🔥',
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
...
㋛${prefix}sider [reply chat RAFHAN SER]
...
case 'script':
		case 'sc':
		case 'sourcecode':
		RAFHAN SER.sendMessage(from, { text: "https://github.com/DGRAFHAN SER/RAFHAN SERRAFHAN SER", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
//if (!isRAFHAN SERGroupAdmins) return reply(mess.only.Badmin)
if (isBanchat) return reply(`already banned`)
bancht.push(from)
fs.writeFileSync('./database/banchat.json', JSON.stringify(bancht))
reply(`Successful RAFHAN SER Ban on this group`)
break
...
bancht.splice(ubc, 1)
fs.writeFileSync('./database/banchat.json', JSON.stringify(bancht))
reply(`The RAFHAN SER has been unbanned in this group`)
break
...
case 'linkgroup':
              ...
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
                   if (!isRAFHAN SERGroupAdmins) return reply('RAFHAN SER not admin')
         ...
         if (!isGroupAdmins) return reply('this feature is only for admin')
                   if (!isRAFHAN SERGroupAdmins) return reply('RAFHAN SER isnt admin')
...
break
                   ...
                   if (!isRAFHAN SERGroupAdmins) return reply('RAFHAN SER is not admin')
     ...
     break
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await RAFHAN SER.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						RAFHAN SER.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *RAFHAN SER BROADCAST* 」\n\n${ini_bc}`})
					}
...
if (!isGroup) return reply(`*Only group*`)
             ...
             break
      case "sider":
        if (!m.quoted) throw `Reply to RAFHAN SER messages!`;
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
			...
			if (!isRAFHAN SERGroupAdmins) return sticNotAdmin(from)
...
break
      case "rentRAFHAN SER":
        if (!mek.key.fromMe) return reply("Cant be a rentRAFHAN SER in a RAFHAN SER");
        jadiRAFHAN SER(reply, RAFHAN SER, from);
        break;
      case "stoprentRAFHAN SER":
        if (!mek.key.fromMe)
          return reply("cant stop being RAFHAN SER except the owner");
        stopjadiRAFHAN SER(reply);
        break;
      case "listRAFHAN SER":
        let tekss = "「 *LIST RENTRAFHAN SER* 」\n";
        for (let i of listjadiRAFHAN SER) {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          ...
          "url": "https://github.com/DGRAFHAN SER/RAFHAN SERRAFHAN SER"
        },
        "index": 0
      }
    ]
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             RAFHAN SER.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 RAFHAN SER BROADCASTING 」*\n\n${body.slice(4)}`})
}
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply('only admin can use this feature')
					...
					if (Number(args[0]) === 1) {
						...
						RAFHAN SER.sendMessage(from, `Free to Find Hentai 🗿`, text)
			...
			case 'googletxt2':
					...
					break
			case 'battlefield': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} RAFHAN SER|RAFHAN SER`)
					makell = args.join(" ")
					ll1 = makell.split("|")[0];
					ll2 = makell.split("|")[1];
					reply(mess.wait)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
      ...
      case "slander":
        if (args.length < 1)
          return reply(
            `Usage :\n${prefix}slander [@tag|message|replyRAFHAN SER]]\n\nEx : \n${prefix}slander @tagmember|hi|hi too`
          );
        var gh = args.join("");
        mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid;
        var replace = gh.split("|")[0];
        var target = gh.split("|")[1];
        var RAFHAN SER = gh.split("|")[2];
        RAFHAN SER.sendMessage(from, `${RAFHAN SER}`, text, {
          quoted: {
            key: {
              fromMe: false,
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        dua = typeof anu[1] !== "undefined" ? anu[1] : `RAFHAN SER`;
...
if (!isOwner)return// reply(`This command can not be carried out by rent RAFHAN SER temporarily`)
if (!q)return 
var itsme = `${sender}`
var split = `*RAFHAN SER RAFHAN SER Inc.*`
const term = {
contextInfo: {
participant: itsme,
quotedMessage: {
...
RAFHAN SER.sendMessage(from, stdout, text, term)
      ...
      default:
		if (hour_now >= '02:00' && hour_now <= '04:00') {
          console.log(color('[Message RAFHAN SER]', 'cyan'), color('Horny = Bonks', 'yellow'), color('(💥)', 'white'))
          }
    ...
    if (isGroup && budy != undefined) {
      ...
      console.log(
        color("[RAFHAN SER RAFHAN SER]", "red"),
        "RAFHAN SER",
        color(sender.split("@")[0])
      );
    }
